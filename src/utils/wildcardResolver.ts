...
export class WildcardResolver {
  ...
  static resolve(
    ...
    for (const pattern of patterns) {
      ...
      for (const candidate of candidates) {
        const match = this.matchPattern(pattern, candidate, patternType, opts);
        if (match) {
          results.push(match);
        }
      }
  ...
  static buildFtrackConditions(
    ...
    for (const pattern of patterns) {
      ...
      if (patternType === 'exact') {
        ...
        const sqlPattern = this.toSqlLike(pattern);
  ...
  private static detectPatternType(pattern: string): 'exact' | 'wildcard' | 'regex' {
  ...
  static fuzzyMatch(pattern: string, candidate: string): number {
    ...
    const distance = this.levenshteinDistance(patternLower, candidateLower);
...
export class InteractiveResolver {
  ...
  static analyzePattern(
    ...
    if (matchCount === 0) {
      suggestions = this.suggestPatterns(pattern, candidates);
