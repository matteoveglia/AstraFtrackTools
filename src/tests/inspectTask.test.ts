...
describe('inspectTask', () => {
    ...
    it('should process task details with provided taskId', async () => {
        ...
        expect(mockSession.query).toHaveBeenNthCalledWith(1, expect.stringContaining('task-1'));
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('TASK DETAILS'));
    ...
    it('should handle empty results gracefully', async () => {
        ...
        expect(mockSession.query).toHaveBeenCalledTimes(3);
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('TASK DETAILS'));
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('TIME LOGS'));
        expect(console.log).toHaveBeenCalledWith(expect.stringContaining('VERSIONS'));
    });
});