...
Deno.test("DeletionService - should handle dry run for asset versions", async () => {
	...
	const result = await deletionService.deleteAssetVersions(["version-1"], {
		dryRun: true,
	});
...
Deno.test("DeletionService - should handle dry run for components", async () => {
	...
	const result = await deletionService.deleteComponents(componentChoiceMap, {
		dryRun: true,
	});
...
Deno.test("DeletionService - should handle component deletion choices", async () => {
	...
	const allResult = await deletionService.deleteComponents(allChoiceMap, {
		dryRun: true,
	});
	...
	const singleResult = await deletionService.deleteComponents(singleChoiceMap, {
		dryRun: true,
	});
...
Deno.test("DeletionService - should handle permission errors", async () => {
	const deletionService = new MockDeletionService();
	const result = await deletionService.deleteAssetVersions(["error-version"], {
		dryRun: false,
	});
