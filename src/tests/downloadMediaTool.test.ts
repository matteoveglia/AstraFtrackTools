...
import type { AssetVersion, Component, Shot } from "../types/mediaDownload.ts";
...
Deno.test("FilterService - should build date filter correctly", () => {
	...
	assertEquals(whereClause, 'date >= "2024-01-01" and date <= "2024-12-31"');
});
...
Deno.test("Integration - should combine all filter types", async () => {
	...
	assertEquals(whereClause.includes('status.name in ("Approved")'), true);
	assertEquals(whereClause.includes('user.username in ("testuser")'), true);
	assertEquals(whereClause.includes('date >= "2024-01-01"'), true);
	assertEquals(whereClause.includes('date <= "2024-12-31"'), true);
	assertEquals(
...
Deno.test("Edge Case - should handle special characters in filter values", () => {
	...
	assertEquals(whereClause.includes('Status with "quotes"'), true);
});
...
Deno.test("Edge Case - should handle invalid date formats", () => {
	...
	assertEquals(whereClause, 'date >= "invalid-date" and date <= "2024-12-31"');
});
...
Deno.test("AssetVersion Filtering - should filter by status", async () => {
	...
	const result = await queryService.queryAssetVersions(
		`asset.parent.id is "shot-1" and ${whereClause}`,
	);
...
Deno.test("AssetVersion Filtering - should filter by user", async () => {
	...
	queryService.queryAssetVersions = async (query: string) => {
		if (query.includes('user.username in ("john.doe")')) {
			return await Promise.resolve({
				data: mockAssetVersionData.filter(
					(v) => v.user?.username === "john.doe",
				),
			});
		}
		return await Promise.resolve({ data: mockAssetVersionData });
...
Deno.test("AssetVersion Filtering - should combine filters correctly", async () => {
	...
	queryService.queryAssetVersions = async (query: string) => {
		...
		if (
			query.includes('status.name in ("Approved")') &&
			query.includes('user.username in ("john.doe")')
		) {
			return await Promise.resolve({
				data: mockAssetVersionData.filter(
					(v) =>
...
Deno.test("AssetVersion Filtering - should handle no matching versions", async () => {
	...
	const result = await queryService.queryAssetVersions(
		'asset.parent.id is "nonexistent-shot"',
	);
