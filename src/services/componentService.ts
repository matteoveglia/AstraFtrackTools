...
export class ComponentService {
	 ...
	 */
	async getComponentsForAssetVersion(
      ...
      `;
			...
			if (!result.data || result.data.length === 0) {
				throw new Error(
					`Asset version not found: ${assetVersionId}`,
				) as InvalidAssetVersionError;
			}
			...
			if (!assetVersion.components || assetVersion.components.length === 0) {
				throw new Error(
					`No components found for asset version: ${assetVersionId}`,
				) as ComponentNotFoundError;
			}

			debug(
				`Found ${assetVersion.components.length} components for asset version ${assetVersionId}`,
			);
			return assetVersion.components;
		} catch (error) {
			debug(
				`Failed to query components for asset version ${assetVersionId}: ${error}`,
			);
			throw error;
		}
	}
	...
	async getDownloadUrl(componentId: string): Promise<string | null> {
		try {
			...
			const session =
				this.sessionService.getSession() as SessionWithGetComponentUrl;
			...
			debug(
				`Error getting download URL for component ${componentId}: ${error}`,
			);
			return null;
		}
	}
	...
	findBestComponent(
		...
		debug(
			`Component types found: ${Array.from(componentsByType.keys()).join(
				", ",
			)}`,
		);
		...
		for (const type of fallbackChain) {
			...
			if (componentsOfType && componentsOfType.length > 0) {
				...
				debug(
					`Selected component: ${bestComponent.name} (type: ${type}, size: ${
						bestComponent.size || "unknown"
					})`,
				);
	 ...
	 */
	async getAssetVersionWithComponents(
		assetVersionId: string,
      ...
      `;
			...
			const assetVersion = result.data[0] as AssetVersion;
			debug(
				`Retrieved asset version: ${assetVersion.asset.name} v${assetVersion.version}`,
			);

			return assetVersion;
		} catch (error) {
			debug(
				`Failed to get asset version with components ${assetVersionId}: ${error}`,
			);
			throw error;
		}
	}
	...
	formatComponentTypeDisplay(component: Component): string {
		...
		if (baseType === "image" && component.file_type) {
			...
			const cleanFileType = component.file_type
				.replace(/^\.+/, "")
				.toUpperCase();
