...
export class ListService {
    ...
    `);
		...
		const entityIds = (listObjectsResponse?.data || [])
			.map((lo: Record<string, unknown>) => lo.entity_id)
      ...
      `);
			...
			const chunkIds = (avResponse?.data || [])
				.map((av: Record<string, unknown>) => av.id as string)
				.filter((id: unknown): id is string => typeof id === "string");
