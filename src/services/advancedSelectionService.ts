...
export class AdvancedSelectionService {
	...
	async selectAssetVersions(
		...
		console.log(
			chalk.green("Features: Search, Wildcards, Fuzzy Matching, Pagination"),
		);
	...
	private async searchBasedSelection(
		...
		console.log(
			"  • Wildcards: SHOT* (prefix), *_v001 (suffix), SH*_v* (multiple)",
		);
	...
	private async listBasedSelection(
		...
		const listItems: SelectionItem[] = lists.map((list) => {
			...
			const category = listRecord.category as
		...
		if (versionIds.length === 0) {
			console.log(
				chalk.yellow("No asset versions found in the selected list."),
			);
			return { items: [], cancelled: true };
		}
		...
		for (const pattern of patterns) {
			if (pattern.startsWith("/") && pattern.endsWith("/")) {
				try {
					...
					console.log(
						chalk.yellow(`Warning: Invalid regex pattern: ${pattern}`),
					);
				}
			}
		}
		...
		const items: SelectionItem[] = (
			...
			return {
				...
				metadata: {
					...
					assetName,
					version: versionNum,
					status: (version.status as Record<string, unknown> | undefined)
						?.name as string,
					user: (version.user as Record<string, unknown> | undefined)
						?.username as string,
				},
			};
		});
	...
	private async paginatedSelection(
		...
		const pageSize =
			options.pageSize || AdvancedSelectionService.DEFAULT_PAGE_SIZE;
		let currentPage = 0;
		...
		while (true) {
			...
			const itemChoices = pageItems.map((item) => ({
				name:
					options.showMetadata && item.description
			...
			const selectedIds = selection.filter(
				(id) =>
					![
						"search",
						"clear_search",
	 ...
	 */
	private async fetchAssetVersionDetails(
		versionIds: string[],
	): Promise<SelectionItem[]> {
	...
	private async getSuggestions(patterns: string[]): Promise<string[]> {
		try {
			...
			for (const pattern of patterns) {
				const patternSuggestions = InteractiveResolver.suggestPatterns(
					pattern,
					candidates,
				);
