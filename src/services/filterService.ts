    ...
    if (custom?.length) {
      for (const f of custom) {
        switch (f.op) {
          case "true":
            clauses.push(
              `custom_attributes any (key is "${f.key}" and value is true)`,
            );
            break;
          case "false":
            clauses.push(
              `custom_attributes any (key is "${f.key}" and value is false)`,
            );
            break;
          case "eq":
            clauses.push(
              `custom_attributes any (key is "${f.key}" and value is ${
          ...
          case "neq":
            clauses.push(
              `not (custom_attributes any (key is "${f.key}" and value is ${
                typeof f.value === "string" ? `"${f.value}"` : f.value
              }))`,
            );
            break;
          case "contains":
            clauses.push(
              `custom_attributes any (key is "${f.key}" and value like "%${f.value}%")`,
            );
            break;
        }
      }
