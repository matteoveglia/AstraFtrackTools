	...
	return {
		updateProgress(processed: number, total: number, etaMs?: number) {
			...
			if (etaMs !== undefined && etaMs > 0) {
				...
				const minutes = Math.floor(etaSeconds / 60);
				const seconds = etaSeconds % 60;
				const etaFormat = `${minutes.toString().padStart(2, "0")}:${seconds
					.toString()
					.padStart(2, "0")}`;
				s += ` (eta ${etaFormat})`;
			}
	...
	try {
		...
		while (processed < total) {
			...
			for (const item of chunk) {
				const sizeMB =
					item.size != null ? (item.size / (1024 * 1024)).toFixed(2) : "";
				const row = [
					item.operation || "",
					item.assetVersionId || "",
			...
			const etaMs =
				avgPerItem > 0 ? Math.round(avgPerItem * remaining) : undefined;
...
async function selectFromList(
	...
	const versionIds =
		await listService.getAssetVersionIdsFromList(selectedListId);
...
export async function deleteMediaTool(
	...
	if (mode === "versions") {
		...
		const inputMethod = await Select.prompt({
			...
			options: [
				...
				{
					name: "🔍 Advanced selection (search, wildcards, filters)",
					value: "advanced",
				},
			],
		});
		...
		if (inputMethod === "ids") {
			...
			if (result.searchUsed && result.patterns) {
				console.log(
					chalk.green(
						`Selected ${result.items.length} asset versions using patterns: ${result.patterns.join(", ")}`,
					),
				);
			} else {
				console.log(
					chalk.green(`Selected ${result.items.length} asset versions`),
				);
			}
		...
		if (proceed) {
			...
			console.log(
				`Total size freed: ${DeletionService.formatBytes(
					deletionResult.summary.bytesDeleted,
				)}`,
			);
	...
	if (mode === "components") {
		...
		if (inputMethod === "ids") {
			...
			if (selectedVersions.includes("all")) {
				versionIds = (result.data as Array<{ id: string }>).map(
					(v) => v.id as string,
				);
			} else {
	...
	if (mode === "filter") {
		...
		const versionIds = (avResult.data as Array<{ id: string }>).map(
			(v) => v.id,
		);
		console.log(
			chalk.green(
