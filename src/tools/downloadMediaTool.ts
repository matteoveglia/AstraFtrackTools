...
async function handleSingleVersionDownload(
	...
	await debugToFile(DEBUG_LOG_PATH, "Version ID entered:", versionId);
...
async function configureShotFilters(): Promise<{
	...
	if (filterTypes.includes("status")) {
		const statusNames = await Input.prompt({
			message:
				"Enter status names (comma-separated, e.g., 'In Progress,Review'):",
	...
	if (filterTypes.includes("user")) {
		const usernames = await Input.prompt({
			message:
				"Enter usernames (comma-separated, e.g., 'john.doe,jane.smith'):",
			validate: (input: string) => {
				if (!input.trim()) {
...
async function handleMultipleShotsDownload(
	...
	const applyVersionFilters = await Confirm.prompt({
		message:
			"Would you like to apply filters to narrow down version selection?",
		default: false,
	});

	// Configure version filters if requested
	const versionFilters = applyVersionFilters
		? await configureVersionFilters()
		: null;
	...
	for (const shot of matchingShots) {
		...
		if (filteredVersions.length > 0) {
			...
			if (versionFilters) {
				const versionList = filteredVersions
					.map((v) => `v${v.version}`)
					.join(", ");
	...
	const uniqueShots = new Set(shotsWithVersions.map((item) => item.shot.id))
		.size;
...
> {
		...
		const batchPromises = batch.map(async ({ shot, latestVersion }) => {
			...
			try {
				...
				if (!result.success) {
					...
					const components =
						await componentService.getComponentsForAssetVersion(
							latestVersion.id,
						);
					return {
						shot,
	...
	console.log(
		`Results: ${successCount}/${totalShots} successful (${Math.round(
			(successCount / totalShots) * 100,
		)}%)`,
	);
	...
	try {
		...
		if (versionsResult?.data && versionsResult.data.length > 0) {
			...
			if (!filters) {
				...
				const assetTypes = versions
					.map((av) => av.asset?.type?.name)
					.filter(Boolean);
				await debugToFile(
					DEBUG_LOG_PATH,
				...
				for (const mediaType of mediaTypes) {
					const mediaVersion = versions.find(
						(av) => av.asset?.type?.name === mediaType,
					);
					if (mediaVersion) {
						await debugToFile(
							DEBUG_LOG_PATH,
...
async function processAssetVersion(
	...
	try {
		...
		if (!bestComponent) {
			...
			return {
				success: false,
				reason: `No suitable component found for preference: ${mediaPreference}`,
			};
		}
...
async function handleAutomaticFallback(
	...
	for (const type of fallbackPriority) {
		...
		if (componentsOfType && componentsOfType.length > 0) {
			...
			if (type === "other") {
				const imageComponent = componentsOfType.find(
					(c) =>
						c.file_type &&
