...
async function addShotsToList(
  ...
  `);
	...
	const notFoundCodes = parsedCodes.filter(
		(code: string) => !foundShotNames.includes(code),
	);
  ...
  `);
	...
	const shotsToAdd = foundShots.filter(
		(shot) => !existingEntityIds.includes(shot.id),
	);
